...
export const StudentPreviewPane: React.FC<StudentPreviewPaneProps> = ({
  ...
  const [mathModalSuggestedFilename, setMathModalSuggestedFilename] = useState<string | null>(
    null
  );
  const [filenamePrefix, setFilenamePrefix] = useState<string>(() => {
    try {
      return localStorage.getItem('flvs_math_filename_prefix') || 'ap_stats_';
    } catch {
      return 'ap_stats_';
    }
  });
  ...
  const handleFilenamePrefixChange = (val: string) => {
    ...
    try {
      localStorage.setItem('flvs_math_filename_prefix', val);
    } catch {
      // ignore storage errors (e.g. private browsing)
    }
  };

  const handleSaveFocus = (e: React.SyntheticEvent<HTMLElement>) => {
    const el = e.currentTarget;
    lastFocusedNodeRef.current = el;
    setLastFocusedNode(el);
  ...
  // Figure out which box (question stem or a specific answer choice) an
  // element belongs to, based on the data-* attributes we stamp onto the
  // contentEditable boxes below.
  const computeSuggestedFilenameSuffix = (node: HTMLElement): string | null => {
    const field = node.getAttribute('data-field');
    const qNum = node.getAttribute('data-question-number');
    if (!field || !qNum) return null;

    let boxLabel: string | null = null;
    if (field === 'prompt') {
      boxLabel = 'stem';
    } else if (field === 'choice') {
      const letter = node.getAttribute('data-choice-letter');
      boxLabel = letter ? letter.toLowerCase() : null;
    }
    if (!boxLabel) return null;

    const base = `q${qNum}_${boxLabel}_flvs`;

    // Avoid collisions: scan filenames already used inside this box and
    // auto-increment (_2, _3, ...) if this base name is already taken.
    const existingInputs = Array.from(
      node.querySelectorAll('.filename-input')
    ) as HTMLInputElement[];
    ...
    return count === 0 ? base : `${base}_${maxIndex + 1}`;
  };

  const handleOpenMathModal = () => {
    const node = lastFocusedNodeRef.current;
    const valid = node && document.body.contains(node) && node.getAttribute('data-field');

    if (!valid) {
      alert('Please click inside a question stem or answer choice box first, then reopen the MathJax Lab.');
      return;
    }

    setMathModalSuggestedFilename(computeSuggestedFilenameSuffix(node as HTMLElement));
  ...
  // Insert HTML from MathLab Modal into active or target question/choice
  const handleInsertMathHtml = (htmlString: string) => {
    let targetNode = lastFocusedNodeRef.current || lastFocusedNode;

    // Fallback: search for first contenteditable element in DOM if none focused
    if (!targetNode || !document.body.contains(targetNode)) {
      targetNode = document.querySelector('[contenteditable="true"]') as HTMLElement | null;
    }

    if (!targetNode) {
      alert('Please click inside a question prompt or choice option first.');
      return;
    }

    // 1. Re-focus target element
    targetNode.focus();

    // 2. Restore or create selection range
    const selection = window.getSelection();
    if (selection) {
      ...
      if (
        ...
        targetNode.contains(savedRangeRef.current.commonAncestorContainer)
      ) {
        rangeToUse = savedRangeRef.current;
      } else {
        rangeToUse = document.createRange();
        rangeToUse.selectNodeContents(targetNode);
        rangeToUse.collapse(false); // caret at end
      }
      try {
    ...
    // 3. Perform execCommand insertHTML or DOM append
    let inserted = false;
    try {
    ...
    // 4. Instantly synchronize innerHTML with React state so it persists
    const updatedHtml = targetNode.innerHTML;
    const stagedId = targetNode.getAttribute('data-staged-id');
    const field = targetNode.getAttribute('data-field');
    const choiceId = targetNode.getAttribute('data-choice-id');

    if (stagedId && field) {
      const targetItem = stagedItems.find((item) => item.id === stagedId);
      if (targetItem) {
        if (field === 'prompt') {
          handlePromptChange(stagedId, targetItem.question, updatedHtml);
        } else if (field === 'choice' && choiceId) {
          handleChoiceTextChange(stagedId, targetItem.question, choiceId, updatedHtml);
        }
      }
    }
  };
  ...
  return (
    <div id="visual-editor-container" className="bg-slate-50/70 rounded-2xl border border-slate-200 p-4 sm:p-6 mb-6">
      ...
      {stagedItems.length === 0 && (
        <div className="bg-white rounded-2xl border-2 border-dashed border-slate-300 p-8 sm:p-12 text-center my-6 shadow-2xs">
          ...
          <div className="flex items-center justify-center space-x-3 flex-wrap gap-y-2">
            ...
            <button
              onClick={() => {
                setMathModalSuggestedFilename(null);
                setIsMathModalOpen(true);
              }}
                              ...
                              <div className="mb-5">
                                ...
                                {isRawCode ? (
                                  ...
                                  <div
                                    ...
                                    data-field="prompt"
                                    data-question-number={globalIndex}
                                    onFocus={handleSaveFocus}
                                    onClick={handleSaveFocus}
                                    onKeyUp={handleSaveFocus}
                                    onBlur={(e) => {
      ...
      <MathLabModal
        ...
        suggestedFilename={mathModalSuggestedFilename}
        filenamePrefix={filenamePrefix}
        onFilenamePrefixChange={handleFilenamePrefixChange}
      />
    </div>
  );
};
